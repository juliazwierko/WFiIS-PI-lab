...
1)void losuj(char *t, int n); - wypełnia wejściową tablicę losowymi bitami, zapisanymi w formie znaków. 
Parametr n oznacza liczbę bitów do wylosowania.

2)void pisz(char *t); - wypisuje tablicę wejściową traktując ją jako łańcuch (bez użycia pętli). 

3)Trzecia funkcja powinna zwracać (nie wypisywać) wartość dziesiętną wylosowanej liczby
binarnej w U1. Wartość potęgi 2^k można obliczyć przy pomocy funkcji z biblioteki
...
#include <stdlib.h>
#include <time.h> 
#include <math.h> // gcc -g -Wall -lm wpisujemy w terminale 
